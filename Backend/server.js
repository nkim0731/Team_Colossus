...
if (process.env.TESTING == 'true') {
    process.env.CLIENT_ID = '444444444444-thisisonlyfortesting.apps.googleusercontent.com';
...
.put(async (req, res) => {
    ...
    const useremail = data.username;
    try {
        const updateResult = await db.updatePreferences(useremail, data.preferences);
...
.post(async (req, res) => {
    ...
    try {
        ...
        const eventAddResult = await db.addEvents(useremail, data.events);
        if (!eventAddResult) return res.status(404).json({ message: 'No user for username: ' + useremail });
